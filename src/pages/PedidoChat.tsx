...
const PedidoChat = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      } else if (meta.payment_id) {
        const { data: payment } = await supabase
          .from("payments")
          .select("amount, coupon_id")
          .eq("id", meta.payment_id)
          .maybeSingle();
        if (payment) {
          setValorPago(Number((payment as any).amount) / 100);
          const cid = (payment as any).coupon_id;
          if (cid) {
            const { data: coupon } = await supabase.from("coupons").select("code").eq("id", cid).maybeSingle();
            if (coupon) setCouponCode((coupon as any).code);
          }
        }
      }
  ...
  const linkify = (text: string) => {
    ...
    return text.split(urlRegex).map((part, i) =>
      part.match(urlRegex) ? (
        <a key={i} href={part} target="_blank" rel="noopener noreferrer" className="text-success underline hover:text-success/80 break-all">{part}</a>
      ) : (
        part
      )
    );
  ...
  const [expandedSection, setExpandedSection] = useState<string | null>("done");

  useEffect(() => {
    if (stockContent) setExpandedSection("delivery");
  }, [stockContent]);
