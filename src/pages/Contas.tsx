...
const regionTranslationMap: Record<string, string> = {
  ...
  "Europa Norte/Leste": "Europa Norte/Leste",
  "LAS": "LAS",
  "LAN": "LAN",
  "Oceania": "Oceania",
  "Turquia": "Turquia",
...
const translateRegion = (regionName: string | undefined): string => {
  if (!regionName) return "";
  // Try exact match first
  if (regionTranslationMap[regionName]) {
    return regionTranslationMap[regionName];
  }
  // Try case-insensitive match
  const lowerRegion = regionName.toLowerCase();
  for (const [key, value] of Object.entries(regionTranslationMap)) {
    if (key.toLowerCase() === lowerRegion) {
      return value;
    }
  }
  // If no translation found, return original (might be already in Portuguese)
  return regionName;
};
    ...
    >
      ...
      <div className="p-5">
        ...
        {item.riot_lol_region && (
          <div className="mt-2 flex items-center gap-1.5">
            <Globe className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="text-[11px] text-muted-foreground">{translateRegion(item.riot_lol_region)}</span>
          </div>
        )}
...
const fetchAccountsRaw = async (params: Record<string, string | string[]>) => {
  ...
  try {
    // Get session token if available, otherwise use anon key as Bearer token
    // Supabase may require Authorization header even with verify_jwt = false
    const { data: { session } } = await supabase.auth.getSession();
    const bearerToken = session?.access_token || anonKey;
    
    const url = `${projectUrl}/functions/v1/lzt-market?${queryParams.toString()}`;
            ...
            {streamError && (
              <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                ...
                <button
                  ...
                  className="mt-4 rounded border border-border px-4 py-2 text-xs font-medium transition-colors hover:bg-secondary"
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = accentColor; (e.currentTarget as HTMLElement).style.color = accentColor; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = ''; (e.currentTarget as HTMLElement).style.color = ''; }}
                >
                  Tentar novamente
