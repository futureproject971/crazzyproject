      ...
      {loading ? (
        ...
        <motion.div
          className={`mt-14 grid gap-6 ${
            games.length === 1
              ? "max-w-sm mx-auto grid-cols-1 place-items-center"
...
const Produtos = () => {
  ...
  // Preserve a search started from the new CRAZZY home header.
  useEffect(() => {
    const query = searchParams.get("q") || "";
    if (query !== searchQuery) setSearchQuery(query);
    // Intentionally react only to the URL: typing in the local filter must not rewrite itself.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]);

  // Keep the artistic home selector and this catalogue route in sync.
  // The URL carries the category slug, so cards remain real links instead of
  // hiding navigation rules inside the visual component.
  useEffect(() => {
    if (games.length === 0) return;
    const gameSlug = searchParams.get("game");
    if (!gameSlug) return;
    const matched = games.find((game) => game.slug === gameSlug);
    if (matched && matched.id !== selectedGame) setSelectedGame(matched.id);
  ...
  const selectGame = (gameId: string) => {
    ...
    if (game?.slug) {
      const next: Record<string, string> = { game: game.slug };
      const query = searchParams.get("q");
  ...
  if (!selectedGame) {
    return <GameSelectScreen onSelect={selectGame} games={games} loading={loadingGames} />;
  }
