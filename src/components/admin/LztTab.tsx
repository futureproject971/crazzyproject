...
const LztTab = () => {
  ...
  // Lucro esperado por conta (baseado em max_fetch_price e markup médio)
  const avgMarkup = config
    ? (Number(config.markup_valorant || config.markup_multiplier) +
       Number(config.markup_lol ?? config.markup_multiplier) +
       Number(config.markup_fortnite ?? config.markup_multiplier) +
       Number(config.markup_minecraft ?? config.markup_multiplier)) / 4
    : 1.5;
  const maxFetchBrl = (config?.max_fetch_price || 500) * 0.055; // RUB→BRL
  const expectedProfitPerAccount = maxFetchBrl * (avgMarkup - 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        <StatCard icon={<TrendingUp className="h-5 w-5 text-amber-400" />} label="Lucro Esperado/Conta" value={`R$ ${expectedProfitPerAccount.toFixed(2)}`} />
