...
const COLORS = ["hsl(218,100%,50%)", "hsl(220,80%,55%)", "hsl(45,100%,55%)", "hsl(0,80%,55%)"];
...
const FinanceTab = () => {
  ...
  const pieData = Object.entries(methodCounts)
    .filter(([, v]) => v > 0)
    .map(([key, value]) => ({
      name: key === "pix" ? "PIX" : key === "card" ? "Cartão" : key === "crypto" ? "LTC" : key,
      value,
    }));
      ...
      <body>
        ...
        <table>
          ...
          ${Object.entries(methodCounts)
            ...
            .map(([key, count]) => `
              <tr>
                <td>${key === "pix" ? "PIX" : key === "card" ? "Cartão" : "LTC"}</td>
                <td>${count}</td>
        ...
        <p class="footer">CRAZZY PROJECT · Relatório gerado automaticamente</p>
      </body>
      </html>
    `;
          ...
          {barData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={barData}>
                ...
                />
                <Bar dataKey="valor" fill="hsl(218,100%,50%)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={monthlyData()}>
              ...
              <Bar dataKey="contas" name="Contas" fill="hsl(218,100%,50%)" radius={[4, 4, 0, 0]} stackId="a" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
