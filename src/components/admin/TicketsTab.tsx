...
const TicketsTab = () => {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    if (data) {
      ...
      const paymentIds = [...new Set(data.map((t: any) => t.metadata?.payment_id).filter(Boolean))];
      ...
      const [productsRes, plansRes, profilesRes, lztSalesRes, paymentsRes] = await Promise.all([
        ...
        paymentIds.length > 0 ? supabase.from("payments").select("id, amount, coupon_id").in("id", paymentIds) : { data: [] },
      ]);

      let couponMap: Record<string, string> = {};
      const couponIds = [...new Set((paymentsRes.data || []).map((p: any) => p.coupon_id).filter(Boolean))];
      if (couponIds.length > 0) {
        const { data: coupons } = await supabase.from("coupons").select("id, code").in("id", couponIds);
        (coupons || []).forEach((c: any) => { couponMap[c.id] = c.code; });
      }
      const paymentMap: Record<string, { amount: number; coupon_code?: string }> = {};
      (paymentsRes.data || []).forEach((p: any) => {
        paymentMap[p.id] = { amount: p.amount, coupon_code: p.coupon_id ? couponMap[p.coupon_id] : undefined };
      });
      ...
      setTickets(data.map((t: any) => {
        ...
        const planPrice = isLzt ? lztPrice : (planMap[t.product_plan_id]?.price ?? 0);
        const paymentInfo = meta?.payment_id ? paymentMap[meta.payment_id] : null;
        const displayPrice = paymentInfo ? (paymentInfo.amount / 100).toFixed(2) : Number(planPrice).toFixed(2);
        const couponCode = paymentInfo?.coupon_code;
  ...
  const [expandedSection, setExpandedSection] = useState<string | null>("done-admin");
    ...
    return (
      <div className="space-y-2">
        {parts.map((part, i) => {
          ...
          const fileName = isTutorial ? "tutorial.txt" : `arquivo.${ext.toLowerCase()}`;
          ...
          return (
            <button
              ...
              className="flex w-full items-center gap-3 rounded-xl border border-border bg-background/80 p-3 transition-all hover:border-success/40 hover:bg-success/5 group text-left cursor-pointer"
            >
                ...
                <button onClick={() => selectTicket(ticket)} className="w-full text-left">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-1">
                    ...
                    <span className="font-medium text-foreground/70">
                      R$ {(ticket as any).display_price ?? Number(ticket.plan_price || 0).toFixed(2)}
                      {(ticket as any).coupon_code ? ` (cupom ${(ticket as any).coupon_code})` : ""}
                    </span>
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 ml-10">
                      ...
                      <span className="text-xs font-medium text-foreground/70">
                        R$ {((selectedTicket as any).display_price ?? Number(selectedTicket.plan_price || 0).toFixed(2))}
                        {(selectedTicket as any).coupon_code ? ` (cupom ${(selectedTicket as any).coupon_code})` : ""}
                      </span>
                    </div>
                  </div>
