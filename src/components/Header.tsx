...
const Header = () => {
  ...
  const isCrazyHome = location.pathname === "/";
  ...
  const accentColor = isCrazyHome ? (isDark ? "#4D7CFF" : "#0000FF") : isLolContext ? LOL_BLUE : "hsl(var(--success))";
  const homeText = isDark ? "#EDF4FF" : "#07142f";
  const homeMuted = isDark ? "#9EB3D8" : "#33466b";
      ...
      <motion.header
        ...
        style={{
          ...
          background: scrolled
            ...
            : isCrazyHome
              ? isDark
                ? "linear-gradient(180deg, rgba(3,8,20,.72), rgba(3,8,20,0))"
                : "linear-gradient(180deg, rgba(255,255,255,.66), rgba(255,255,255,0))"
              : "transparent",
          backdropFilter: scrolled ? "blur(20px) saturate(1.5)" : isCrazyHome ? "blur(8px)" : "none",
          boxShadow: scrolled ? (isCrazyHome ? (isDark ? "0 10px 34px rgba(0,0,0,.28)" : "0 10px 30px rgba(0,55,150,.08)") : "0 4px 30px hsla(0,0%,0%,0.3)") : "none",
        }}
      >
        <div className="mx-auto flex h-16 lg:h-[72px] max-w-7xl items-center justify-between px-4 lg:px-6">
          ...
          <Link to="/" className="flex items-center gap-2.5 lg:gap-3 shrink-0 group">
            <motion.span
              className="grid h-9 w-9 lg:h-11 lg:w-11 place-items-center rounded-lg border text-[11px] lg:text-xs font-black italic tracking-[-0.08em]"
              style={{
                borderColor: isCrazyHome ? "rgba(0,0,255,.18)" : `${accentColor}35`,
                color: accentColor,
                background: isCrazyHome ? (isDark ? "rgba(14,27,53,.78)" : "rgba(255,255,255,.78)") : "hsl(var(--secondary))",
                boxShadow: isCrazyHome ? "0 8px 24px rgba(0,67,255,.12)" : "none",
              }}
            ...
            <span
              className="text-lg lg:text-xl tracking-[0.14em] whitespace-nowrap"
              style={{ fontFamily: "'Valorant', sans-serif" }}
            >
              <span style={{ color: isCrazyHome ? homeText : "hsl(var(--foreground))" }}>CRAZZY</span>
              <span style={{ color: accentColor }}> PROJET</span>
            </span>
          </Link>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    color: active ? accentColor : isCrazyHome ? homeMuted : "hsl(var(--muted-foreground))",
                  }}
                  onMouseEnter={e => {
                    if (!active) (e.currentTarget as HTMLElement).style.color = isCrazyHome ? homeText : "hsl(var(--foreground))";
                  }}
                  onMouseLeave={e => {
                    if (!active) (e.currentTarget as HTMLElement).style.color = isCrazyHome ? homeMuted : "hsl(var(--muted-foreground))";
                  }}
                >
                  {item.label}
              ...
              {user ? (
                ...
                <div className="px-4 py-4 border-t border-border/50 flex flex-col gap-3">
                  ...
                  <button
                    ...
                    className="w-full bg-success py-2.5 text-sm font-semibold text-success-foreground rounded-md transition-all hover:shadow-[0_0_24px_hsl(218,100%,50%,0.45)] btn-shine"
                  >
                    Criar Conta
                  </button>
                </div>
