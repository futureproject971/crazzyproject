...
export async function calculateServerTotal(
  ...
  for (const rawItem of cartItems) {
    ...
    if (isLztAccount) {
      ...
      try {
        const detailRes = await fetch(`https://api.lzt.market/${encodeURIComponent(lztItemId)}`, {
        ...
        const currency = String(detailData.item?.price_currency || "rub").toLowerCase();
        ...
        const priceBrl = currency === "rub" ? lztPriceRaw * RUB_TO_BRL : lztPriceRaw;
        const unitPriceCents = Math.round(priceBrl * markup * 100);
    ...
    const { data: planData, error: planError } = await supabaseAdmin
      .from("product_plans")
      .select("id, name, price, active, product_id")
  ...
  if (couponId) {
    ...
    if (coupon.discount_type === "percentage") {
      ...
      discountAmount = Math.round(couponBaseCents * (percentage / 100));
    } else if (coupon.discount_type === "fixed") {
      discountAmount = Math.min(couponBaseCents, Math.round(Number(coupon.discount_value || 0) * 100));
    } else {
  ...
  return { total: Math.round(total), subtotal, discountAmount, cartSnapshot, couponId: validatedCouponId };
}
